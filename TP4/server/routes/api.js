...
router.post("/login", async (req, res) => {
  ...
  // si on a pas trouvé l'utilisateur,alors on le crée
...
/*
 * Cette route  retourne le panier de l'utilisateur, grâce à req.session
 */
...
/*
 * Cette route  ajoute un article au panier, puis retourner le panier modifié à l'utilisateur
...
router.post("/panier", (req, res) => {
  ...
  if (isNaN(quantity) || quantity <= 0) {
    res.status(400).json({
      message:
        "Vous ne pouvez pas ajouter une quantité  qui est nulle dans votre panier",
    });
    return;
  }
  ...
  if (!article) {
    res
      .status(404)
      .json({ message: "L'article n'existe pas donc doit etre creer" });
    return;
  }
...
router.post("/panier/pay", (req, res) => {
  ...
  if (
    ...
    res.status(400).json({ message: "request" });
    return;
  }
  req.session.destroy();
...
/**
 * Cette route push les livres vers le site
 */
